...
class Program
{
        ...
        try
        {
            for (int i = 0; i < args.Length; ++i)
            {
                ...
                if (arg == "--hex-subfile-names")
                    ...
                    options.Add(Option.UnicodeNormalization);
    ...
    static void Import(string gameDataPath, string inputPath, List<Option> options, List<string> files)
    {
        ...
        bool ProcessFiles(string containerFile, Func<SortedDictionary<uint, string>, bool> processor, bool zeroBased, bool withMultipleTextsForSubFiles = false)
        {
            ...
            {
                string text = File.ReadAllText(path, Encoding.UTF8).TrimEnd('\r', '\n');

                if (options.Contains(Option.UnicodeNormalization))
                {
            ...
            var files = Directory.GetFiles(directory, "*.txt")
                ...
                .Where(f => fileNameMatcher.IsMatch(f.name));
            var fileEntries = new SortedDictionary<uint, string>(files.ToDictionary(f => indexAdjust(fileIndexParser(f.name)), f => ReadFile(f.path)));
    ...
    private static string ReplaceSpecialLetters(string input)
    {
        return input
            .Replace("œ", "oe")
            .Replace("Œ", "OE")
